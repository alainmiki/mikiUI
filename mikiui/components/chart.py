...
from .html import Div, Svg, Svg as _Svg


class Chart(Component):
    """A lightweight SVG chart.

    ``kind`` is ``"line"``, ``"bar"``, or ``"pie"``. ``series`` is a list of
    numeric values. This is a basic renderer; richer charts are widgets.
    """
    ...
    def __init__(
        ...
        **attrs: Any,
        ...
        if not series:
            super().__init__(Div("(no data)"), **attrs)
            return
        if kind == "bar":
    ...
    @staticmethod
    def _bars(series: list[float], w: int, h: int) -> _Svg:
        n = len(series)
        gap = 4
        ...
        for i, v in enumerate(series):
            x = gap + i * (bw + gap)
            bh = (v / maxv) * (h - 10)
            rects.append(f'<rect x="{x:.1f}" y="{h - bh:.1f}" width="{bw:.1f}" height="{bh:.1f}"></rect>')
        return Svg("".join(rects), viewBox=f"0 0 {w} {h}", width=w, height=h)

    @staticmethod
    def _line(series: list[float], w: int, h: int) -> _Svg:
        n = len(series)
        maxv = max(series) or 1
        pts = []
        for i, v in enumerate(series):
        ...
        return Svg(
            f'<polyline points="{" ".join(pts)}" fill="none" stroke="currentColor"></polyline>',
            viewBox=f"0 0 {w} {h}",
            width=w,
            height=h,
        )
    ...
    @staticmethod
    def _pie(series: list[float], w: int, h: int) -> _Svg:
        ...
        for v in series:
            ...
            paths.append(
                f'<path d="M{cx:.1f},{cy:.1f} L{x1:.1f},{y1:.1f} A{r:.1f},{r:.1f} 0 {large} 1 {x2:.1f},{y2:.1f} Z"></path>'
            )
            angle = a2
